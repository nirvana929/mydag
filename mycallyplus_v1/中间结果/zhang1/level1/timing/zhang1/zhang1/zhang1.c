...
SEG_BEGIN("THR:c2_fn#001@118-129");
    ...
    return NULL;
SEG_END("THR:c2_fn#001@118-129");
}

static void *c1_fn(void *arg)
{
SEG_BEGIN("THR:c1_fn#001@134-136");
    pthread_mutex_lock(&mutex4);
    busy_wait_seconds(C4);
    pthread_mutex_unlock(&mutex4);
SEG_END("THR:c1_fn#001@134-136");
SEG_BEGIN("CRT:c1_fn#002@L137");
    pthread_create(&tc2, NULL, c2_fn, NULL);
SEG_END("CRT:c1_fn#002@L137");
SEG_BEGIN("THR:c1_fn#003@138-148");
        sem_wait(&sem3);
    ...
    sem_post(&sem6);
    return NULL;
SEG_END("THR:c1_fn#003@138-148");
}

static void *c0_fn(void *arg)
{

SEG_BEGIN("THR:c0_fn#001@154-156");
    pthread_mutex_lock(&mutex7);
    busy_wait_seconds(C7);
    pthread_mutex_unlock(&mutex7);
SEG_END("THR:c0_fn#001@154-156");
SEG_BEGIN("CRT:c0_fn#002@L157");
    pthread_create(&tc1, NULL, c1_fn, NULL);
SEG_END("CRT:c0_fn#002@L157");
SEG_BEGIN("THR:c0_fn#003@158-168");
    sem_wait(&sem1);
    ...
    sem_post(&sem4);
    return NULL;
SEG_END("THR:c0_fn#003@158-168");
}


int main(void)
{
SEG_BEGIN("THR:main#001@174-201");
...
SEG_END("THR:main#001@174-201");
SEG_BEGIN("CRT:main#002@L202");
    pthread_create(&tc0, NULL, c0_fn, NULL);
SEG_END("CRT:main#002@L202");
SEG_BEGIN("THR:main#003@203-213");
...
SEG_END("THR:main#003@203-213");
SEG_BEGIN("JON:main#004@L214");
    pthread_join(tc0, NULL);
SEG_END("JON:main#004@L214");
SEG_BEGIN("THR:main#005@215-217");
    pthread_mutex_lock(&mutex14);
    busy_wait_seconds(C14);
    pthread_mutex_unlock(&mutex14);
SEG_END("THR:main#005@215-217");
SEG_BEGIN("JON:main#006@L218");
    pthread_join(tc1, NULL);
SEG_END("JON:main#006@L218");
SEG_BEGIN("THR:main#007@219-221");
    pthread_mutex_lock(&mutex15);
    busy_wait_seconds(C15);
    pthread_mutex_unlock(&mutex15);
SEG_END("THR:main#007@219-221");
SEG_BEGIN("JON:main#008@L222");
    pthread_join(tc2, NULL);
SEG_END("JON:main#008@L222");
SEG_BEGIN("THR:main#009@223-232");
