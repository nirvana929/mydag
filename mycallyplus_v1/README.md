...
ScratchDAG is a toolkit to generate and explore call graphs for C/C++ projects, run time analysis via automatic instrumentation, and experiment with scheduling strategies (longest-path, CPC). It ships with a GUI and CLI, outputs DOT/PNG/JSON artifacts, and keeps results organized for comparison.

## What it does
- Graph generation: parse GCC RTL `.expand` or source; export thread/full DOT/PNG and circle.txt.
- Visualization: viewer/GUI for original, threads, conditions, mutex, semaphore, Tarjan SCC, thread clusters.
- Time analysis: auto-instrument `.c` via `mycalls_meta_internal.json`, compile/run, emit `time_result.json`, `thread_trace.json`, `metrics.json`.
- Scheduling: longest-path, CPC priority assignment, CPC priority-aware instrumentation, metrics comparison.
...
- Mutex / semaphore views (require circle.txt).
- Time analysis: select source + meta → copy, instrument, build, run → results in `中间结果/<base>/时间分析/<project>/`.
- Scheduling:
  - Longest path: `dag.dot` + `time_result.json` → `调度算法/longest_path/`.
  - CPC analysis: `dag.dot` + `time_result.json` + `longest_path.json` → `调度算法/cpc/`.
  - CPC priority instrumentation: source + meta + CPC `schedule.json` → outputs in `时间分析_cpc/<project>/` and `调度算法/cpc分析结果/`.
  - Metrics compare: pick two metrics JSON → comparison PNGs under `调度算法/compare/<timestamp>/`.
- Filter DOT: save `_filt.dot/png` under `中间结果/过滤dot/`.
...
## Scheduling notes
- CPC priorities from `schedule.json.priorities`. Instrumentation sets priority once per thread prefix (first hit) via `ta_set_priority(SCHED_FIFO)`. Without root/CAP_SYS_NICE, calls may fail (EPERM) and only add overhead.
